...
### 3.2.4 T细胞基因在疾病和疫苗中的三层数据集比对

为验证"全血Bulk无法捕捉T细胞编程"这一核心假设, 我们将T细胞生物学关键基因在**三个独立数据集**中进行交叉验证:

| 数据集 | 技术 | T细胞来源 | 比较 | 检测能力 |
|--------|------|---------|------|---------|
| GSE242252 | Bulk RNA-seq | 全血（T细胞信号被稀释） | HZ急性 vs 恢复 | **系统性全景，T细胞信号弱** |
| HRA008316 | scRNA-seq | PBMC中的T细胞 | HZ患者(HP) vs 健康(HA) | **T细胞内在变化，无疫苗对照** |
| GSE249632 | scRNA-seq | gE四聚体⁺ CD4⁺ T细胞 | RZV D14/D74/D365 vs D0 | **抗原特异性T细胞编程** |

52个T细胞关键基因在三数据集中的表现:

| 模式 | 数量 | HRA008316 T细胞 | HZ全血 | RZV T细胞 | 代表基因 |
|------|:--:|:--:|:--:|:--:|---------|
| **RZV可见，全血被掩盖** | **22** | N/A或弱 | ✗ (ns) | ✓ | **ZEB2, CTLA4, ICOS, HAVCR2, TIGIT, GATA3, TOX, CXCL13, IL21** |
| 两个scRNA均可见 | 1 | ✓ | ✗ (ns) | ✓ | GZMA |
| 仅HZ全血可见（非T来源） | 6 | ✗ | ✓ | ✗ | **ISG15, STAT1, MX1, SERPING1** |
| 三者均变化 | 9 | ✓或N/A | ✓ | ✓ | PDCD1, LAG3, TOP2A, MKI67 |
| 三者均稳定 | 14 | N/A | ✗ | ✗ | STAT3, LEF1, TNFRSF9 |

**解读**:

1. **22个基因在RZV T细胞中显著变化，但在HZ全血中完全检测不到** — 包括ZEB2, CTLA4, ICOS, HAVCR2等。这些是Protection Signature的核心成员，在HZ全血中的LFC接近0且不显著，不是因为它们未变化，而是T细胞信号被全血中其他细胞(中性粒细胞占白细胞的50-70%)的RNA系统性稀释。

2. **ISG15, STAT1, MX1等仅在HZ全血中显著** — HRA008316的T细胞数据中这些基因不显著或方向相反(ISG15在HP T细胞中LFC=-0.37)，证实全血中的ISG信号来自非T细胞。

3. **HRA008316作为桥梁验证了T细胞信号的可检测性**: GZMA在HRA008316 T细胞(+0.80***)和RZV T细胞(+2.7)中均显著，但在HZ全血中仅+1.07** — 说明scRNA-seq可以检测到T细胞内在变化。

**该分析的实质**: 全血Bulk RNA-seq (GSE242252) 和 gE-CD4⁺ T细胞scRNA-seq (GSE249632) 之间的关系不是"可比较的两个视角"，而是"捕捉免疫系统两个不同维度的互补窗口"。HRA008316提供了连接两者的中间证据: scRNA-seq确实可以检测到T细胞在HZ中的变化(GZMA, NKG7)，但这些变化在Bulk全血中被稀释。完整数据表见 `results/Tcell_genes_cross_dataset.csv` 和 `results/ThreeDataset_Tcell_comparison.csv`。

### 3.2.5 ISG信号：全血可见，T细胞不可见

**数据与图表**: `results/chapter3_figures/bulk_vs_scRNA_comparison.png`

HZ全血 (GSE242252) 和 RZV gE-CD4⁺ T细胞 (GSE249632) 在ISG表达上呈现鲜明对比:

| 基因 | HZ全血 (GSE242252) | RZV gE-CD4⁺ T (GSE249632) | 差异 |
|------|--------------------|--------------------------|------|
| IFI27 | +2.16 * | −0.1 (ns, D14) | HZ特异性 |
| IFI44L | +1.82 * | −0.3 (ns, D14) | HZ特异性 |
| SERPING1 | +1.80 *** | −0.5 (ns, D14) | HZ特异性 |
| ISG15 | +1.57 ** | +0.1 (ns, D14) | HZ特异性 |
| RSAD2 | +1.55 * | −0.3 (ns, D14) | HZ特异性 |

这一差异的生物学基础: HZ全血包含所有免疫细胞群体, ISG信号来源于病毒感染激活的单核细胞、树突状细胞和中性粒细胞等天然免疫细胞——这一推断得到独立scRNA-seq数据 (HRA008316, Zheng et al. 2024) 的初步支持: Classical Monocyte中IFI27表达在HZ患者中为健康对照的27.6倍。CD4⁺ T细胞本身不是I型IFN的主要产生者, 因此gE特异性CD4⁺ T细胞在疫苗应答中不表达ISGs是**细胞类型固有的属性**, 而非疫苗刻意"避免"的结果。

**该对比的意义**: 它说明HZ全血中的ISG程序 (Disease Reference) 和RZV特异性T细胞转录程序 (Protection Reference) 代表免疫应答中**两个不同的细胞层面**——前者是系统性固有免疫激活, 后者是抗原特异性T细胞内在分化编程。候选疫苗的评价需要分别在这两个层面进行, 而非要求T细胞表现出固有免疫的特征。
...
| **Disease Reference** | ISG15, RSAD2, IFI44L, IFI27, SERPING1, MZB1 — 代表HZ急性期全血中被系统性激活的免疫程序 |
| **用途** | 作为**固有免疫激活"广度"的参照** — 提供HZ疾病中系统性免疫激活的全景图谱 |

> **关键限定**: 上述ISG程序在HZ急性期是宿主抗病毒防御的正常组成部分, 不预设其有害性。这些基因在RZV gE-CD4⁺ T细胞中的缺失反映的是细胞类型差异 (T细胞非I型IFN主要产生者) 而非疫苗的"回避策略"。HZ全血和RZV T细胞各自捕捉免疫应答的不同层面, 联合使用提供互补信息。
...
1. CD4⁺ T细胞非I型IFN主要产生细胞, 其不表达ISGs是**细胞类型固有的属性**, 而非疫苗设计刻意"避免"的结果。
2. GSE242252 (全血) 和 GSE249632 (纯化T细胞) 在ISG上的差异, 本质上反映的是两者捕捉的免疫层面不同: 前者包含ISG产生的天然免疫细胞, 后者不含。
3. 因此, 这两个数据集在ISG问题上的关系是**互补而非对立** — HZ全血展示了系统性ISG激活的全景, RZV T细胞展示了在该全景中"没有T细胞参与ISG生产"这一事实。两者共同说明: 保护性T细胞免疫的建立不需要T细胞自身进入ISG程序。
