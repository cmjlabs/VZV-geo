...
### 3.2.4 T细胞编程基因在HZ全血中被系统性稀释

两个数据集在细胞组成上的差异导致了一个关键的信息不对称: **许多在RZV特异性T细胞中显著变化的基因, 在HZ全血中完全检测不到**。

我们从文献中选取了50个T细胞生物学关键基因 (涵盖TCR信号、分化、共刺激/检查点、细胞毒、记忆、Tfh、转录因子等), 检测它们在两个数据集中的变化:

| 类别 | 基因数 | 在HZ全血中显著 | 在RZV T细胞中显著 | 代表基因 |
|------|:------:|:------------:|:---------------:|---------|
| **全血中被掩盖** | **23** | ✗ (ns) | ✓ (|LFC|>0.58) | ZEB2, CTLA4, ICOS, HAVCR2, TIGIT, GATA3, TOX, CXCL13, IL21, CD4 |
| 两者均变化 | 5 | ✓ | ✓ | GZMA, PDCD1, LAG3, TNFRSF4, CD38 |
| 两者均稳定 | 14 | ✗ | ✗ | STAT3, STAT4, CD28, ITK, LEF1 |
| 其他 | 8 | — | — | 未在任一数据集中检测到 |

**全血中被掩盖的23个基因示例**:

| 基因 | 功能 | HZ全血 LFC | HZ p值 | RZV D14 | RZV D365 | 
|------|------|-----------|--------|---------|----------|
| ZEB2 | T细胞效应-记忆分化 | −0.11 | ns | **+3.2** | **+3.0** |
| CTLA4 | 免疫检查点 | +0.28 | ns | **+1.5** | **+1.0** |
| ICOS | T细胞共刺激 | +0.14 | ns | **+1.3** | +0.6 |
| HAVCR2 | TIM-3/效应记忆 | +0.05 | ns | **+7.6** | **+8.0** |
| GATA3 | Th2主调控因子 | +0.14 | ns | **+1.0** | +0.9 |
| TOX | 耗竭/分化调控 | −0.04 | ns | **+1.3** | +0.5 |
| CXCL13 | Tfh趋化因子 | +0.05 | ns | **+3.3** | **+2.5** |
| IL21 | Tfh效应因子 | +0.64 | ns | **+1.8** | **+1.9** |
| CD4 | T细胞共受体 | −0.09 | ns | +0.9 | +0.4 |

**该结果的实质**: 这些基因的HZ全血LFC接近0且不显著, **不是因为它们在HZ中不变化**, 而是T细胞信号在全血中被大量其他细胞的RNA稀释。全血Bulk RNA-seq对T细胞内在编程的检测灵敏度极低——这正是需要scRNA-seq数据的原因。
